            ...
            if (xhr.status === 200) {
                try {
                    ...
                    if (response.exists !== undefined) {
                        ...
                        throw new Error("Unexpected response format: 'exists' key is missing. Response: " + xhr.responseText);
                    }
                } catch (error) {
                    console.error('Error occurred while parsing response:', error);
                    alert('Error occurred while parsing response. Please try again.');
                }
            } else {
                console.error('Error occurred while checking SKU existence. Status:', xhr.status);
                alert('Error occurred while checking SKU existence. Please try again.');
            }
        }
    };
    xhr.open('POST', 'Actions/check_sku.php');
...
function saveProduct() {
    ...
    xhr.open('POST', 'Actions/process_product.php');
    xhr.setRequestHeader('Content-Type', 'application/json');
